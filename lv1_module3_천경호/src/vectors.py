...
def row_echelon(A, pivoting: bool = True):
    ...
    for i in range(m):
        if i>=n: break #마지막 열까지 왔다면 멈춤
        #피벗 찾기
        candidate_row = i+np.argmax(np.abs(a[i:,i]))
        if np.abs(a[i,candidate_row])<tol: # 피벗이 0에 가까우면 이번 열은 스킵
            continue
        ...
        if pivoting:
            pivot_cols.append(i)
            if candidate_row != i:
                a[[i,candidate_row]]=a[[candidate_row,i]]
                n_swaps+=1
