...
def test_columns_are_orthonormal(maker, theta):
    ...
    R=maker(theta)
    assert orthogonality_error(R) == pytest.approx(0,abs=1e-12)
...
def test_determinant_is_one(maker, theta):
    # TODO: det(R) == 1 인지 검사
    R=maker(theta)
    assert det(R) == pytest.approx(1.0,abs=1e-12)
...
def test_inverse_equals_transpose(maker, theta):
    ...
    R=maker(theta)
    assert np.linalg.inv(R)==pytest.approx(R.T,abs=1e-12)
...
def test_gram_schmidt_restores_orthogonality(rng):
    ...
    R=rodrigues([1,1,1],np.pi/4)

    R= R + rng.random(size=(R.shape))*1e-10
    R = gram_schmidt(R)
    assert det(R) == pytest.approx(1.0,abs=1e-12)
    assert is_rotation(R) == True
